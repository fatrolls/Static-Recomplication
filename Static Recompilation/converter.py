...
def to_c(format_spec, *args):
    new_args = []
    for arg in args:
        if TypeOf arg Is str:
            arg = StringWrapper(arg)
        elif TypeOf arg Is bool:
            if arg:
                arg = 'true'
            else:
                arg = 'false'
...
class CodeWriter(object):
    ...
    def put_method(self, name, *arg, **kw):
        fullarg = list(arg)
...
OT_a = &H01000000
OT_b = &H02000000        # always 1 byte
OT_c = &H03000000        # byte or word, depending on operand
OT_d = &H04000000        # double-word
OT_q = &H05000000        # quad-word
OT_dq = &H06000000       # double quad-word
OT_v = &H07000000        # word or double-word, depending on operand
OT_w = &H08000000        # always word
OT_p = &H09000000        # 32-bit or 48-bit pointer
OT_pi = &H0a000000       # quadword MMX register
OT_pd = &H0b000000       # 128-bit double-precision float
OT_ps = &H0c000000       # 128-bit single-precision float
OT_s = &H0d000000        # 6-byte pseudo descriptor
OT_sd = &H0e000000       # Scalar of 128-bit double-precision float
OT_ss = &H0f000000       # Scalar of 128-bit single-precision float
OT_si = &H10000000       # Doubleword integer register
OT_t = &H11000000        # 80-bit packed FP data
...
AM_A = &H00010000  # Direct address with segment prefix
AM_I = &H00060000  # Immediate data follows
AM_J = &H00070000  # Immediate value is relative to EIP
AM_I1 = &H00200000 # Immediate byte 1 encoded in instruction
    ...
    def __init__(self, inst, op, offset):
        ...
        if self.type == pydasm.OPERAND_TYPE_REGISTER:
            ...
            if inst.dispbytes:
                ...
                if op.displacement & (1<<(op.dispbytes*8-1)):
                    ...
                    if op.dispbytes == 1:
                        ...
                        tmp = Not tmp & &Hffffffff
                    else:
                        raise NotImplementedError()
                    self.displacement = '(-&H%x)' % (tmp + 1)
                # Positive displacement
                else:
                    self.displacement = '&H%x' % op.displacement
            ...
            if mask == AM_J:
                ...
                value = op.immediate & &HFFFFFFFF
            # 32-bit or 48-bit address
            elif mask == AM_A:
                raise NotImplementedError()
                string += '&H%x:&H%x' % (op.section, op.displacement)
...
class Instruction(object):
    ...
    def group2(self):
        return (self.flags & &H00ff0000) >> 16

    def operand_so(self):
        return bool(self.flags & &H00000100)
    ...
    def address_so(self):
        return bool(self.flags & &H00001000)

    def get_address_mode(self):
        if self.flags & &H00001000:
            return MODE_16
        else:
            return MODE_32
...
class Subroutine(object):
    ...
    def __init__(self, converter, address):
        ...
        while True:
            ...
            if hasattr(instruction, 'eflags_used'):
            ...
            if hasattr(instruction, 'eflags_affected'):
    ...
    def is_end(self, inst, index):
        ...
        if mnemonic in ('retn', 'ret', 'int3'):
            return True
        if mnemonic == 'jmp' and opcode in (&HFF,):
            # far jumps
            return True
...
class CPU(object):
    ...
    def on_call(self, i):
        test_ebp = False
        # if self.eip in (&H54ABD5, &H54AC12):
        #     test_ebp = False
    ...
    def on_jmp(self, i):
        ...
        if (i.opcode in (&HEB, &HE9) and op.value >= self.sub.start
    ...
    def __init__(self, path):
        ...
        for entry in pe.DIRECTORY_ENTRY_IMPORT:
            for imp in entry.imports:
                ...
                print 'imp.address = %X' % imp.address
        ...
        # writer.put_method('void init_emu')
        ...
        for address in addresses:
            ...
            writer.putln('cpu.add_function(&H%X, %s);' % (address, name))
        writer.end_brace()
